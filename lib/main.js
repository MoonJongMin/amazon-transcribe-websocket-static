...
let sourceLanguageCodeForTranscription;
let region;
let sampleRate;
let inputSampleRate;
let transcription = "";
...
let translate;
let translation = "";
let sourceLanguageCodeForTranslation;
let targetLanguageCodeForTranslation;
...
function setLanguage() {
    sourceLanguageCodeForTranscription = $('#sourceLanguage').find(':selected').val();
    // Transcribe 은 en-US 와 같은 형식이지만 Translate 는 en, ko 라고만 해야 한다.
    sourceLanguageCodeForTranslation = sourceLanguageCodeForTranscription.substring(0, 2);
    let targetLanguageCode = $('#targetLanguage').find(':selected').val();
    // Transcribe 은 en-US 와 같은 형식이지만 Translate 는 en, ko 라고만 해야 한다.
    targetLanguageCodeForTranslation = targetLanguageCode.substring(0, 2);
    if (sourceLanguageCodeForTranscription == "en-US" || sourceLanguageCodeForTranscription == "es-US")
        sampleRate = 44100;
    else if (sourceLanguageCodeForTranscription == "ko-KR")
        sampleRate = 30000;
    else
        sampleRate = 8000;
}
...
function createPresignedUrl() {
    ...
    return v4.createPresignedURL(
        ...
        crypto.createHash('sha256').update('', 'utf8').digest('hex'), {
            ...
            'expires': 15,
            'region': region,
            'query': "language-code=" + sourceLanguageCodeForTranscription + "&media-encoding=pcm&sample-rate=" + sampleRate
        }
    );
}
...
function doTranslate(text, isPartial) {
    ...
    // Source, target language 가 동일하면 Transacibe 만 수행하고 Translate 는 하지 않도록 한다.
    if (!text || sourceLanguageCodeForTranslation === targetLanguageCodeForTranslation) {
    ...
    let params = {
        Text: text,
        SourceLanguageCode: sourceLanguageCodeForTranslation,
        TargetLanguageCode: targetLanguageCodeForTranslation
    };
    ...
    translate.translateText(params, function(err, data) {
        ...
        if (data) {
            ...
            if(!isPartial) {
                ...
                let tempTraslation = translation;
                translation += data.TranslatedText + "\n"
                // 300ms delay 를 주어서 다시 dispay 하도록하는 이유는 위 주석 참고...
                setTimeout(() => $('#translation').val(tempTraslation + data.TranslatedText + "\n"), 300);
                // translation += data.TranslatedText + "\n";
            }
        }
    });
